...
__global__ void computeMatMul(double* A, double* B, double* C, int N, int M, int P, int offset) {
	...
	for (int row=bid*offset; row<(bid+1)*offset; row++) {
		if (row >= N) {
			break;
		}
		...
		for (int col=0; col<M; col++) {
			...
			for (int i=tid; i<P; i+=blockDim.x) {
				tmp += A[row*P+i]*B[i*M+col];
			}
			// Atomic add
			atomicAdd(&C[row*M+col], tmp);
		}
	}
}
...
int main(int argc, char **argv) {
	...
	computeMatMul<<<nbBlocks, nbThreads>>>(gpu_A, gpu_B, gpu_C, Ndim, Mdim, Pdim, offset);
